...
for tissue in tissueType:
    ...
    for muspPath in muspPathSet:
        ...
        if tissue != "blood":
            df.musp = df.musp/10  # convert 1/cm to 1/nn when tissue's type is not "blood".
    ...
    for idx, name in enumerate(musp.keys()):
        ...
        if tissue == "blood":
            interpData[tissue][name] = calculateMusp(wlProject, *ab[idx])/(1-0.95)
        else:
            interpData[tissue][name] = calculateMusp(wlProject, *ab[idx])/(1-0.9)
    ...
    # save mus
    musRange[tissue] = [np.ceil(maxMus), np.floor(minMus)] #divided by 10 to convert to mm^-1
...
for tissue in interpData.keys():
    for wl in output_wavelength:
        ...
        output_parameter[f'{wl}nm'][tissue] = [np.nanmax(get_values), np.nanmin(get_values)] #divied by 10 to convert to mm^-1
