...
for tissue in interpData.keys():
    ...
    muaRange[tissue] = [np.ceil(np.nanmax(allmua)*1e3)/1e3/10, np.floor(np.nanmin(allmua)*1e3)/1e3/10] #divided  by 10 to convert to mm^-1
    print(f"{tissue}'s mua ---> min={np.nanmin(muaRange[tissue])}, max={np.nanmax(muaRange[tissue])}")
    ...
    for wl in output_wavelength:
        ...
        if tissue == "ijv":
            for k in interpData[tissue].keys():
                bloodConc = k.split()[-3]
                bloodSO2 = k.split()[-1]
                output_parameter[f'{wl}nm'][f'{tissue}_bloodConc_{bloodConc}_bloodSO2_{float(bloodSO2):.2f}'] = list(interpData[tissue][k][find_wl_idx]/10) #divied by 10 to convert to mm^-1
        get_values = pd_interpData.iloc[find_wl_idx].to_numpy()
        output_parameter[f'{wl}nm'][tissue] = [np.nanmax(get_values)/10, np.nanmin(get_values)/10] #divied by 10 to convert to mm^-1
