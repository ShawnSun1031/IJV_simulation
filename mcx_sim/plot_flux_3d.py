# -*- coding: utf-8 -*-
"""
Created on Sun Aug 28 18:30:39 2022
...
for t in range(flux.shape[0]):
    ...
    for i in range(flux.size):
        ...
        if flux[sort_flux[0][i],sort_flux[1][i],sort_flux[2][i]] >= threshold:
            ...
            cmap = plt.cm.jet(1*flux[sort_flux[0][i],sort_flux[1][i],sort_flux[2][i]]/threshold)
            # print(plt.cm.hsv(flux[sort_flux[0][i],sort_flux[1][i],sort_flux[2][i]]/threshold))
        cmap = list(cmap)
        if flux[sort_flux[0][i],sort_flux[1][i],sort_flux[2][i]] >= threshold:
            cmap[-1] = 1
        else:
            cmap[-1] = flux[sort_flux[0][i],sort_flux[1][i],sort_flux[2][i]]/threshold 
        cmap = tuple(cmap)
