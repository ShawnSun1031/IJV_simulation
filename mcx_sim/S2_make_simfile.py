...
for run_idx in range(1,mus_set.shape[0]+1):
    ...
    if not os.path.isdir(os.path.join(sessionID,run_name)): 
        os.mkdir(os.path.join(sessionID,run_name))
    for filename in copylist:
        src = f'{filename}'
        dst = os.path.join(sessionID,run_name,src) 
        ...
        if src == "config.json":
            ...
            config["MCXInputPath"] = os.path.join(os.getcwd(),sessionID,run_name,"ijv_dense_symmetric_detectors_backgroundfiber_pmc.json")
            config["OutputPath"] = os.path.join(os.getcwd(),sessionID)
