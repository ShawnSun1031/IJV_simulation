...
# -*- coding: utf-8 -*-
"""
Created on Sun May 22 21:28:09 2022
...
def calculate_R_CV(sessionID,session,muaPath):
    ...
    result["GroupingSampleValues"] = {f"sds_{detectorIdx}": group_reflectance[:, detectorIdx].tolist() for detectorIdx in range(group_reflectance.shape[1])}
    
    
    result["GroupingSampleValues"] = {f"sds_{detectorIdx}": group_reflectance[:, detectorIdx].tolist() for detectorIdx in range(group_reflectance.shape[1])}
    result["GroupingSampleStd"] = {f"sds_{detectorIdx}": std[detectorIdx] for detectorIdx in range(std.shape[0])}
    result["GroupingSampleMean"] = {f"sds_{detectorIdx}": mean[detectorIdx] for detectorIdx in range(mean.shape[0])}
    result["GroupingSampleCV"] = {f"sds_{detectorIdx}": CV[detectorIdx] for detectorIdx in range(CV.shape[0])}
