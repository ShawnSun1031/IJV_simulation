...
for run_idx in range(1,mus_set.shape[0]+1):
    ...
    for filename in copylist:
        ...
        if src == "config.json":
            ...
            config["VolumePath"] = os.path.join(os.getcwd(),"ultrasound_image_processing","KB_perturbed_small_to_large.npy")
        ...
        if src == "ijv_dense_symmetric_detectors_backgroundfiber_pmc.json":
            ...
            if sessionID == "KB_ijv_large_to_small":
                mcxInput["Domain"]["Media"][7]["mus"] = mus_set[run_idx-1][3]
            elif sessionID == "KB_ijv_small_to_large":
