# -*- coding: utf-8 -*-
"""
Created on Sun Aug 28 00:46:33 2022
...
path = os.path.join("KB_ijv_small_to_large", "LUT", "run_1", "mcx_output", "run_1_0.jnii")
flux = jd.load(path)
flux = flux['NIFTIData']
flux = flux.T


plt.imshow(flux[:,:,int(flux.shape[2]//2)], cmap=plt.cm.jet, vmax=0.0001, vmin=0)
plt.colorbar()
plt.show()
...
plt.imshow(flux[:,int(flux.shape[1]//2),:], cmap=plt.cm.jet, vmax=5000, alpha=0.4)
plt.axis('off')
plt.colorbar()
...
# sagittal plance
plt.imshow(flux[:,:,int(flux.shape[2]//2)], cmap=plt.cm.jet, vmax=50000, vmin=0)
plt.axis('off')
plt.colorbar()
plt.savefig("pic\\sagittal_flux.png")
plt.show()

plt.imshow(flux[:,:,int(flux.shape[2]//2)], cmap=plt.cm.jet, vmax=30000, vmin=0, alpha=0.8)
plt.axis('off')
plt.colorbar()
